...
func main() {
	...
	counter := prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "example_delta",
			Help: "An example delta counter with dynamic labels",
		},
		[]string{"label_id"},
	)
	...
	// Wrap the handler to simulate OTLP delta metrics: after each scrape, reset
	// all counters so they are recreated with a new creation/start timestamp on
	// the next observation, advancing the delta reporting window.
